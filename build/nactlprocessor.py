...
# Modified by Tianyun Zhang, 2018-10-24
...
def addref(caption, outstream):
    ...
    print(r"\nactlref{%s}" % caption, file=outstream)
...
def processwithcomments(caption, instream, outstream, listingslang = None):
    ...
    out = []
    if warning:
        out.append(r"\nactlwarning{%s: %s}" % (caption, warning))
    if error:
        out.append(r"\nactlerror{%s: %s}" % (caption, error))
    else:
...
def processraw(caption, instream, outstream, listingslang = 'raw'):
    try:
        ...
        print("\nactlerror{Could not read source.}", file=outstream)
